...
.open $SIM
.select network
alarmburst:id=1325783869821767, num_alarms=$NUM_ALARMS,freq=$FREQUENCY_PER_SECOND_PER_NODE,idle_time=$TIME_INTERVAL_BETWEEN_ALARMS,cease_after=1, mo_class="Equipment",mo_instance="STN=0,Equipment=0%unique{4}", type=4,cause=315,severity=3, problem="no_value",text="no_value", loop=true, mode=persistent;
EOF
