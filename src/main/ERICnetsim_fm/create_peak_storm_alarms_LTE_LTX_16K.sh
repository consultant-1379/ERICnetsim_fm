...
SIMLIST=$(ls /netsim/netsimdir | grep '[-]ST[-]FDD-LTE[0-9][0-9]$')
...
FM_AVERAGE_LTE_LIST="LTE16 LTE17 LTE18 LTE19 LTE05 LTE06 LTE07 LTE08"
...
average()
{
    ...
    #The below nodes are used to keep alarms active for > 15 mins to keep in line with the TERE
		...
		burst $LTE $CURR_ID 40 0 60 true 0.02655
...
eems to produce this"
    ...
    LIST2=`makeRbsList $FM_PEAKLTE 44 50`
    LIST3=`makeRbsList $FM_PEAKLTE 37 43`
    LIST4=`makeRbsList $FM_PEAKLTE 31 36`

    # burst RNC ID CEASE_AFTER RNCRATRE NUM_ALARMS LOOP ERBSRATE
    burst $FM_PEAKLTE 110 180 0 277 false 0.9213
...
.select ${LIST1}
alarmburst:freq=${MY_ERBS_FREQ},num_alarms=${MY_NUM_ALARMS},loop=${MY_LOOP},idle_time=0,mo_class="no_value",mo_instance="%mibprefix,ManagedElement=1,Equipment=1,Subrack=MS,Slot=%unique{1000}", system_dn="%mibprefix", cause=1, severity=2, problem="AlmDevice_ExternalAlarm",text="Aircraft Light Facility",type = ET_COMMUNICATIONS_ALARM, cease_after=1,clear_after_burst=true,id=${MY_ID};

.select ${LIST2}
alarmburst:freq=${MY_ERBS_FREQ},num_alarms=${MY_NUM_ALARMS},loop=${MY_LOOP},idle_time=0,mo_class="no_value",mo_instance="%mibprefix,ManagedElement=1,Equipment=1,Subrack=MS,Slot=%unique{1000}", system_dn="%mibprefix", cause=1, severity=3, problem="TU Synch Reference Loss of Signal",type = ET_COMMUNICATIONS_ALARM, cease_after=1,clear_after_burst=true,id=${MY_ID};

.select ${LIST3}
alarmburst:freq=${MY_ERBS_FREQ},num_alarms=${MY_NUM_ALARMS},loop=${MY_LOOP},idle_time=0,mo_class="no_value",mo_instance="%mibprefix,ManagedElement=1,Equipment=1,Subrack=MS,Slot=%unique{1000}", system_dn="%mbprefix", cause=1, severity=2, problem="LinkFailure",type = ET_COMMUNICATIONS_ALARM, cease_after=1, clear_after_burst=true,id=${MY_ID};

.select ${LIST4}
alarmburst:freq=${MY_ERBS_FREQ},num_alarms=${MY_NUM_ALARMS},loop=${MY_LOOP},idle_time=0,mo_class="no_value",mo_instance="%mibprefix,ManagedElement=1,Equipment=1,Subrack=MS,Slot=%unique{1000}", system_dn="%mibprefix", cause=1, severity=1, problem="NssSynchronization_SystemClockStatusChanged",type = ET_COMMUNICATIONS_ALARM, cease_after=1, clear_after_burst=true,id=${MY_ID};
